...
# --------------------------------------------
def imread_uint(path, n_channels=1): #CHANGE n_channels=3 for RGB image ################################# PHONG CHANGE
    #  input: path
...
# SSIM
# --------------------------------------------
def calculate_BER(img1, img2, border=0):
    '''calculate BER
    img1, img2: [0, 255]
    '''
    if not img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    h1, w1 = img1.shape[:2]
    img1 = img1[border:h1-border, border:w1-border]
    img2 = img2[border:h1-border, border:w1-border]
    h, w = img1.shape[:2]
    if img1.ndim == 2:
        img1 = np.reshape(img1,(1,h*w))
        img2 = np.reshape(img2,(1,h*w))
        bit_map1 = np.unpackbits(img1,axis=0)
        bit_map2 = np.unpackbits(img2,axis=0)
        return (1 - np.equal(bit_map1,bit_map2).sum()/(h*w*8))
    elif img1.ndim == 3:
        img1 = np.reshape(img1,(1,h*w*3))
        img2 = np.reshape(img2,(1,h*w*3))
        bit_map1 = np.unpackbits(img1,axis=0)
        bit_map2 = np.unpackbits(img2,axis=0)
        return (1 - np.equal(bit_map1,bit_map2).sum()/(h*w*3*8))
    else:
